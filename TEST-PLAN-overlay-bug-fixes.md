...
| A5 | Tutorials landing page (flip-card carousel): let the overlay appear | **No rectangle/shadow on the center flip card.** Background is dimmed but not blurred | **Fail (partial):** rectangle flickers briefly as the overlay first opens, then goes away. Needs a follow-up fix. User also saw the carousel keep auto-cycling under the overlay; CurvedCarouselV2 autoplay now holds while the overlay is open (retest: carousel pauses, flicker remains). Skipping the fade-in there removed the flicker but looked abrupt, so the backdrop now fades its background colour instead of its opacity on that page; panel fade restored (retest: flicker gone, but dim too quick and panel popped). Carousel page now uses a longer dim and a slower panel ease with no overshoot (retest: smoother, but flicker back). Carousel-page fade-in is now stepped frame by frame from JS with no CSS transitions, same timings. Retest: **Pass** (no flicker, smooth) |
