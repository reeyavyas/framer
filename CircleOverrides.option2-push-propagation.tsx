// CircleOverrides.option2-push-propagation.tsx
//
// VARIANT: "push propagation" corner avoidance.
//
// A circle that's disturbed (dragged, or previously bumped) still keeps
// its own corner/edge pull switched off while a neighbor is physically
// blocking it — same as the shipped file — but now, instead of just
// waiting, it transmits a fraction of its own pull vector onto whichever
// neighbor is in the way, as a "please make room" nudge. The neighbor
// picks that nudge up as its own settle target (waking it up, with its
// own soft fade-in, if it was fully at rest) and drifts aside under it.
// Once there's room, the original circle's own pull resumes and both
// circles end up having moved. No overlap is ever left standing:
// resolveCollisions runs every frame exactly as before and is the only
// thing that's allowed to touch a circle once it's actually overlapping
// a neighbor, so the nudge can only ever move a circle into open space,
// never through another circle.
import React, {
    useEffect,
    useState,
    forwardRef,
...
// and getIncomingNudge below.
const PRESS_MARGIN = 40
// Fraction of a blocked circle's own pull magnitude that gets transmitted
// onto whichever neighbor is standing in its way. Increase for the
// blocking neighbor to yield faster/further; decrease for a subtler,
// slower handoff.
const NUDGE_TRANSMIT_FACTOR = 0.65
// Minimum alignment (dot product, -1..1) between a blocked circle's pull
// direction and the direction toward a neighbor for that neighbor to
...
// it. While that's true, c's own pull stays quiet (see getIncomingNudge
// for what happens instead): easing straight into a neighbor and getting
// shoved back by resolveCollisions every frame is what reads as
// trembling in place.
...
// If another circle is pressed up against `c` and wants to move in a
// direction `c` is blocking, transmit a fraction of that desire onto `c`
// itself — a "please make room" nudge — so the neighbor blocking a
// corner-bound circle also drifts aside, instead of sitting frozen while
...
function getIncomingNudge(c: CircleState): { nudgeX: number; nudgeY: number } {
    ...
    for (const other of circles.values()) {
        ...
        if (dist >= minDist + PRESS_MARGIN) continue // not in contact

        // What `other` wants, based on its own (possibly still-fading-in)
        // corner/edge pull — not whatever's left after its own pull got
        // suppressed by being blocked, since that's exactly the desire we
        // want to hand off to whatever's in its way.
        const otherPull = getEdgePull(
            other.anchorHomeX,
            other.anchorHomeY,
            other.radius
        )
        const pullLen = Math.hypot(otherPull.pullX, otherPull.pullY)
        if (pullLen < 0.01) continue

        const wantX = otherPull.pullX / pullLen
        const wantY = otherPull.pullY / pullLen
        // Direction from `other` toward `c` — is `c` actually in the way
        // of where `other` wants to go, or just incidentally nearby?
        const towardX = (cx - ox) / (dist || 1)
        const towardY = (cy - oy) / (dist || 1)
        const alignment = wantX * towardX + wantY * towardY
        if (alignment <= NUDGE_ALIGNMENT_MIN) continue

        nudgeX += wantX * pullLen * NUDGE_TRANSMIT_FACTOR * alignment
        nudgeY += wantY * pullLen * NUDGE_TRANSMIT_FACTOR * alignment
    }
    ...
    const tick = (timestamp: number) => {
        ...
        circles.forEach((c) => {
            ...
            if (rawProgress < 1) {
                ...
                const incoming = getIncomingNudge(c)
                const incomingMag = Math.hypot(incoming.nudgeX, incoming.nudgeY)
                if (!c.disturbed && incomingMag > 0.01) {
                    // A blocked neighbor is asking this circle to make
                    // room. Wake it the same way a drag or direct
                    // collision would, with its own fresh fade-in so the
                    // yield reads as a soft glide, not a snap.
                    c.disturbed = true
                    c.settledAt = timestamp
                }

                if (c.disturbed) {
                    const blocked = isBlockedByNeighbor(c)
                    const ownPull = blocked
                        ? { pullX: 0, pullY: 0 }
                        : getEdgePull(c.anchorHomeX, c.anchorHomeY, c.radius)
                    ...
                    const settledTargetX =
                        c.anchorHomeX +
                        (ownPull.pullX + incoming.nudgeX) * pullFadeIn
